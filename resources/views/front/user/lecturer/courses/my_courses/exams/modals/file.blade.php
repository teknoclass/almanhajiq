...
<div class="col-12 mb-3">
	<div class="bg-white rounded-2 p-3 item-question">
        <div class="d-flex align-items-center justify-content-between mb-3">
            ...
            @else
            <div class="question-grade font-medium d-flex align-items-center ">
                <input class="form-control me-2" type="number" name="grade[]" placeholder="{{ __('enter_mark') }}"/>
            </div>
            @endif
        </div>
        <div class="row">
            ...
            <div class="col-auto">
                <div class="widget__item-attac">
                    <div class="widget__item-icon">
                        @if ($extension == 'pdf')
                            <a href="{{ CourseAssignmentUrl(@$course->id, @$file) }}" download="{{ $question->title.'.'.$extension }}">
                                <i class="fas fa-file-pdf fa-2x"></i>
                            </a>
                        @else
                            <a href="{{ CourseAssignmentUrl(@$course->id, @$file) }}" download="{{ $question->title.'.'.$extension }}">
                                <img src="{{ CourseAssignmentUrl(@$course->id, @$file) }}" alt="" loading="lazy"/>
                            </a>
                        @endif
                    </div>
                </div>
