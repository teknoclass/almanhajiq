							...
							<table class="table table-cart mb-3">
								...
								<tbody>
                                    ...
                                    @foreach(@$exams as $exam)
                                        @include('front.user.lecturer.courses.my_courses.exams.partials.student_exams')
                                    @endforeach
...
<script>
    $(document).ready(function() {
     $(document).on('click', '.correct_modal', function (e) {
            ...
            $.ajax({
                url: "{{ route('user.lecturer.course.curriculum.get-exam-solution-modal') }}", // Use the new endpoint
                method: "GET",
                data: {
