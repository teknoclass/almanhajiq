...
<div class="row mb-4">
    <div class="col-12" style="display:{{ @$course->is_delete == 1 ? 'none' : 'block'}}">
        @php
        if ($course_item->studentAssignmentResults->isNotEmpty() && !@$passed_time)
            $btn_text = __('continue_assignment');
        else if ($course_item->studentAssignmentResults->isNotEmpty() && @$passed_time)
            $btn_text = __('assignment_result');
        else
            $btn_text = __('start_assignment');

        $btn_link = route('user.courses.curriculum.assignment.start', ['course_id' => @$course->id, 'id' => @$course_item->id]);
        @endphp
