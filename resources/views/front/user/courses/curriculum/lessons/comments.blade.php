@php
    $comments = @$course_item->comments;
    $item_type == 'lesson' ? $comment_type = "lesson" : $comment_type = "live_lesson";
@endphp
<div class="row mb-4">
    <div class="col-12">
        <h4 class="mb-2 circle-before font-medium"><span class="square me-1"></span> {{ __('comments') }}</h4>
    </div>
    <div class="col-12 mb-4" style="display:{{ @$course->is_delete == 1 ? 'none' : 'block'}}">
        <div class="course-comment-input" >
            <form  action="{{ route('user.courses.curriculum.comments.save', @$course->id) }}" to="{{ url()->current() }}" class="commentForm d-flex align-items-center" method="POST">
                @csrf
                <input type="hidden" name="course_id"   value="{{ @$course->id }}">
                <input type="hidden" name="item_id"     value="{{ @$course_item->id }}">
                <input type="hidden" name="item_type"   value="{{ @$comment_type }}">

                <input class="form-control" id="messageBody" name="text" placeholder="{{ __('write_comment') }}">
                <button class="btn p-1 text-white submitForm" type="button"><i class="fa-solid fa-paper-plane-top"></i></button>
            </form>
        </div>
    ...
    <div class="col-12">
        <div class="accordion" id="accordion">
            ...
            <div class="widget__item-message bg-white p-3 rounded-2 flex-column">
                ...
                {{-- content --}}
                <div class="widget__item-body widget__inner accordion-collapse collapse {{ $loop->first ? 'show' : '' }}" id="collapse-{{ $comment->id }}"
                    ...
                    <div class="">
                        <div class="course-comment-input" style="display:{{ @$course->is_delete == 1 ? 'none' : 'block'}}">
                            <form  action="{{ route('user.courses.curriculum.comments.save', @$course->id) }}" to="{{ url()->current() }}" class="commentForm d-flex align-items-center" method="POST">
                                @csrf
                                ...
                                <input class="form-control" id="messageBody" name="text" placeholder="..اكتب تعليقاً">
                                <button class="btn p-1 text-white submitForm" type="button"><i class="fa-solid fa-paper-plane-top"></i></button>
                            </form>
                        </div>
...
<script>
    ...
    $(".commentForm").on("keypress", function(event) {
