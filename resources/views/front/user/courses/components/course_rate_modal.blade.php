    ...
    <div class="modal-dialog modal-dialog-centered">
        ...
        <div class="modal-content">
            <div class="modal-body">
                ...
                <h2 class="font-medium text-center my-3 modals-title">
                    {{ __('add_a_rateing_to') }} {{ @$course->title }}
                </h2>

                <form id="add_rate_form" method="POST" action="{{ route('user.ratings.add') }}" to="{{ url()->current() }}" class="w-100 p-3">
                    @csrf
                    <input type="hidden" name="sourse_id" id="sourse_id"        value="{{ @$course->id }}" />
                    <input type="hidden" name="sourse_type" id="sourse_type"    value="course" />
                    <input type="hidden" name="action_type" id="action_type"    value="course_ratings" />
