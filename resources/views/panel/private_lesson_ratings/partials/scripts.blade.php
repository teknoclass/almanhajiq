...
<script>
    ...
    window.data_url = '{{route('panel.PrivateLessonRatings.all.data')}}';
    window.columns = [{
        ...
        {
            title: '{{__('private_lesson')}}',
            data: function(data) {
                if(data.private_lesson==null){
                    return '';
                }else{
                    return data.private_lesson.title;
                }
            },
        },
        {
...
<script>
    $(document).on('click', '#is_published', function () {
        ...
        $.ajax({
            url: "{{route('panel.PrivateLessonRatings.operation')}}?id=" + item_id,
            method: "post",
            data: {id: item_id},
            success: function (e) {
                if (e.status) {
