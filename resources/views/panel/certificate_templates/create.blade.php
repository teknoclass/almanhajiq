@extends('panel.layouts.index', ['sub_title' => 'الصور', 'is_active' => 'certificate_templates'])
@section('contion')
    @push('panel_css')
                            ...
                            <div class="card card-custom gutter-b example example-compact">
                                <div class="card-header">
                                    ...
                                    <div class="card-toolbar gap-2">
                                        <button type="button"
                                            class="btn btn-primary save-card">{{ __('save') }}</button>

                                        @if (isset($item))
                                            <a href="{{ route('panel.certificateTemplates.certificateTestIssuance', $item->id) }}"
                                                class="btn btn-secondary mr-2">{{ __('view') }}</a>
                                        @endif
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group ">
                                        ...
                                        <select id="course_category_id" name="course_category_id" class="form-control">
                                            ...
                                            @foreach ($course_categories as $course_category)
                                                <option value="{{ @$course_category->value }}"
                                                    {{ @$item->course_category_id == $course_category->value ? 'selected' : '' }}>
                                    ...
                                    <div class="busines-card rounded mb-11">
                                     ...
                                     <button type="button"  class="btn-toplpar" onclick="italicFont()"><i class="fa-solid fa-italic"></i></button> -->
                                                    <input onchange="colorFont(this.value)" type="color"
                                                        class="input-color input-toplpar" value="#0000ff">
                                                </div>
                                            </div>
                                            <div class="d-flex align-items-center justify-content-between gap-2 my-3">
                                                <div class="d-flex gap-2 align-items-center">
                                                    <input type="text" class="form-control input-freetrans"
                                                        placeholder="اضف نص لقالب..." />
                                                    <button style="overflow: visible;" class="btn btn-primary add-freetrans"
                                                        type="button">{{ __('add') }}</button>
                                                </div>
                                                <label for="upload" class="btn btn-primary mb-0">
                                                    <input type="file" id="upload"
                                                        class="input-change-bg-card d-none fileupload">
                                                    <span>{{ __('change_background') }}</span>
                                                </label>
                                            </div>
                                            <div style="text-align: left; color: red; font-weight: bold">(مقاس الصورة يجب أن
                                                يكون كالآتي: عرض 1280 * ارتفاع 904)</div>
                                        </div>
                                        <div class="text-center content-card-front">
                                            <div id="content" class="card-action-front">
                                                <img src="{{ isset($item) ? imageUrl(@$item->background) : '/assets/panel/media/certificate_templete.png' }}"
                                                    alt="" class="bg-card img-fluid2">
                                                @if (isset($item))
                                                    @php
                                                        $texts = $item->texts;
                                                    @endphp
                                                    ...
                                                    font-size:{{ $text->font_size_css }}"
                                                                    ...
                                                                    <div class="template-text"
                                                                        data-type="{{ @$text->type }}"
                                                                        @if (@$text->type == 'others') contenteditable="true" @endif>
                                                                        {!! @$text->text !!}
                                                                    </div>

                                                                    @if (@$text->type == 'others')
                                                                    @endif
                                                                    <div class="remove-text"><i class="fa fa-times"></i>
                                                                    </div>

                                                                    <div class="action-move top-right"><img
                                                                            src="/assets/panel/media/move.svg"
                                                                            alt=""></div>
                                                    ...
                                                    @foreach (config('constants.fixed_texts_templates_certificates') as $text)
                                                        <div class="title-action">
                                                            <div class="freetrans-text freetrans-text-{{ $text['type'] }}"
                                                                ...
                                                                <div class="template-text" contenteditable="false"
                                                                    data-type="{{ @$text['type'] }}"
                                                                    coordinates_left="{{ @$text['left'] }}"
                                                                    coordinates_top="{{ @$text['top'] }}"
                                                                    style="font-size:2vw ;">
                                                                ...
                                                                <div class="action-move top-right">
                                                                    <img src="/assets/panel/media/move.svg"
                                                                        alt="">
                                                                </div>
        ...
        @push('panel_js')
                    ...
                    $.each($(".freetrans-text"), function(key, value) {
                        ...
                        $("#freetrans-text-" + id).resizable({
                            resize: function(event, ui) {
                                if ($(window).width() < 992) {
                                    $(this).find('.template-text').find('*').css("font-size", ui.size.width *
                                        0.020 + "vw");
                                    $(this).find('.template-text').css("font-size", ui.size.width * 0.020 +
                                        "vw")

                                } else {
                                    $(this).find('.template-text').find('*').css("font-size", ui.size.width *
                                        0.011 + "vw");
                                    $(this).find('.template-text').css("font-size", ui.size.width * 0.011 +
                                        "vw")
                                }
                            ...
                            stop: function() {
                                ...
                                var l = (100 * parseFloat($(this).position().left / parseFloat($(this).closest(
                                    '.card-action-front').width()))) + "%";
                                var t = (100 * parseFloat($(this).position().top / parseFloat($(this).closest(
                                    '.card-action-front').height()))) + "%";
                                $(this).css("left", l);
                                $(this).css("top", t);
                            }
                        }).css({
          ...
          <div class="action-move top-right"><img src="/assets/panel/media/move.svg" alt=""></div>
                        ...
                        $(`.freetrans-text-${count}`).resizable({
                            resize: function(event, ui) {
                                if ($(window).width() < 992) {
                                    $(this).find('.template-text').find('*').css("font-size", ui.size
                                        .width * 0.020 + "vw");
                                    $(this).find('.template-text').css("font-size", ui.size.width *
                                        0.020 + "vw")
