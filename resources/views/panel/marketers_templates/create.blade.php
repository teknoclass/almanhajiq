@extends('panel.layouts.index',['sub_title' =>'قوالب المسوقين' ,'is_active'=>'marketers_templates'])
@section('contion')
@push('panel_css')
...
@endphp
@include('panel.layouts.breadcrumb',['breadcrumb_links'=>$breadcrumb_links,'title_page'=>'قوالب المسوقين ',])
<!--begin::Entry-->
               ...
               <div class="card card-custom gutter-b example example-compact">
                  <div class="card-header">
                     ...
                     <div class="card-toolbar">
                        <button type="button" class="btn btn-primary mr-2  save-card">حفظ</button>
                        <a href="{{url()->current()}}" class="btn btn-secondary">إلغاء</a>
                     </div>
                  </div>
                  <!--begin::Form-->
                  <div class="card-body">
                     ...
                     <div class="busines-card rounded mb-11">
                           ...
                           <div class="d-flex align-items-end justify-content-between">
                              <div class="card-toplpar">
                                 ...
                                 <div class="select">
                                    ...
                                    <select class="select-toplpar" onchange="fontSize(this.value)">
                                       <option hidden>اختر ...</option>
                                       @php 
                                       $font_size_value=1.0;
                                       @endphp
                                       @for($font_size=8 ; $font_size<=50 ; $font_size=$font_size+2)
                                       @php 
                                       $font_size_value=$font_size_value + 0.2;
                                       @endphp
                                       <option value="{{@$font_size_value}}">
                                          {{@$font_size}}
                                       </option>
                                       @endfor
                                    </select>
                                 ...
                                 <input  type="color" class="input-color input-toplpar" value="#0000ff">
                              </div>
                           </div>
                           <div class="d-flex my-3">
                              <div class="d-flex align-items-center">
                                 <input type="text" class="form-control input-freetrans" placeholder="اضف نص لقالب..." />
                                 <button class="btn btn-primary ml-2 add-freetrans" type="button">اضف</button>
                              </div>
                              <label for="upload" class="btn btn-primary ml-auto">
                        ...
                        <div class="text-center content-card-front">
                           <div id="content" class="card-action-front">
                              <img src="{{isset($item)?imageUrl(@$item->background):'/assets/panel/media/marketers_templete.jpg'}}" alt="" class="bg-card img-fluid">
                              @if(isset($item))
                              @php
                              $texts=$item->texts;
                              @endphp 
                              ...
                              @endif
                              @else
                              @foreach(config('constants.fixed_texts_templates_marketers') as $text)
                              <div class="title-action">
